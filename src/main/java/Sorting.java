...
public class Sorting {
    ...
    //Taken from stack https://stackoverflow.com/questions/15372705/calculating-a-radius-with-longitude-and-latitude
    public List<DatabaseEntry> coordinateSearch(List<DatabaseEntry> dbListRaw, Double longitude, Double latitude, Double distanceFrom)
    {
        //
        List<DatabaseEntry> dbListSorted = new ArrayList<>();
        ...
        for(DatabaseEntry jobfilter : dbListRaw)
        {
            if(jobfilter.getLatitude() != null && jobfilter.getLongitude() != null)
            {
                ...
                double distanceCalculated = (6371 * Math.acos( Math.sin(latitude1) * Math.sin(latitude2) + Math.cos(latitude1) * Math.cos(latitude2) * Math.cos(longitude2 - longitude1)));
